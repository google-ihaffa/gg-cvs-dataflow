...
import org.apache.beam.DoFn.ConvertJsonToKV;
import org.apache.beam.DoFn.StateDataFilter;
...
public class GgTrailToSpanner {
  ...
  static void runWordCount(WordCountOptions options) {
    ...
    PCollection<KV<Long, String>> pColl =
        p.apply(
                "Read JSON pubsub",
                PubsubIO.readStrings().fromSubscription(options.getSubscriptionsName()))
            // p.apply(Create.of(mockJson))
            .apply("Convert JSON object", ParDo.of(new ConvertJsonToKV()))
            .apply(Reshuffle.of());

    pColl
        .apply(ParDo.of(new StateDataFilter()))
        .apply(
            SpannerIO.write()
