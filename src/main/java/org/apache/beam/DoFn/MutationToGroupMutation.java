...
public class MutationToGroupMutation extends DoFn<KV<Long, Iterable<String>>, MutationGroup> {
  ...
  public void processElement(
      ...
      OutputReceiver<MutationGroup> receiver,
    ...
    for (String jsonString : element.getValue()) {
      ...
      if (json.has("PRESCRIPTION_FILL_ID")) {
        ...
        if (json.getLong("PRESCRIPTION_ID") == 14457820047L || element.getKey() == 14457860479L) {
          LOG.info(
              "PFILL - timestamp of PRESCRIPTION_ID "
                  + element.getKey()
                  + " timestamp "
                  + timestamp
                  + " index timestamp: "
                  + pres_fill_timestamp);
          LOG.info("Current index: " + idx + " result index " + pres_fill_idx);
          LOG.info("Window End: {}", ISODateTimeFormat.dateTime().print(window.maxTimestamp()));
        }
      } else {
        com.google.cloud.Timestamp timestamp =
            DynamicSchemaMapping.convertTimestamp(json.getString(updateTimeKey));
        ...
        if (json.getLong("PRESCRIPTION_ID") == 14457820047L || element.getKey() == 14457860479L) {
          LOG.info(
              "PRES - timestamp of PRESCRIPTION_ID "
                  + element.getKey()
                  + " timestamp "
                  + timestamp);
          LOG.info("Current index: " + idx + " result index " + pres_idx);
          LOG.info("Window End: {}", ISODateTimeFormat.dateTime().print(window.maxTimestamp()));
        }
      }
      list.add(json);
    ...
    if (element.getKey() == 14247357999L) {
    ...
    Mutation.WriteBuilder presFillWriter2 = null;
    ...
    if (pres_idx >= 0) {
      presWriter = Mutation.newInsertOrUpdateBuilder("prescription_uc2_base");
      presWriter =
          prescriptionMutationBuilder(presWriter, list.get(pres_idx), "prescription_uc2_base");
      mutationList.add(presWriter.build());
    }
    if (pres_fill_idx >= 0) {
      presFillWriter = Mutation.newInsertOrUpdateBuilder("prescriptionfill_uc1_second_ih");
      presWriter =
          prescriptionMutationBuilder(
              presFillWriter, list.get(pres_fill_idx), "prescriptionfill_uc1_second_ih");
      mutationList.add(presFillWriter.build());

      // presFillWriter2 = Mutation.newInsertOrUpdateBuilder("prescriptionfill_uc1_first_ih");
      // presWriter =
      //     prescriptionMutationBuilder(
      //         presFillWriter2, list.get(pres_fill_idx), "prescriptionfill_uc1_first_ih");
      // mutationList.add(presFillWriter2.build());
    }
    ...
    //     mutationWriter =
    //         prescriptionMutationBuilder(mutationWriter, jsonObject,
    // "prescriptionfill_uc0_final");
