...
public class JsonToMutation extends DoFn<String, Mutation> {
    ...
    public Mutation.WriteBuilder fillPrescriptionSplit1MutationBuilder(
        ...
        return buildMutationFromMapping(mutationBuilder, prescriptionFillObject, table1ColumnMapping);
    }

    public Mutation.WriteBuilder fillPrescriptionSplit2MutationBuilder(
            Mutation.WriteBuilder mutationBuilder, JSONObject prescriptionFillObject)
            throws JSONException, ParseException {
        return buildMutationFromMapping(mutationBuilder, prescriptionFillObject, table2ColumnMapping);
    }


    private Mutation.WriteBuilder buildMutationFromMapping(
            Mutation.WriteBuilder mutationBuilder,
            JSONObject jsonObject,
            Map<String, Map<String, String>> columnMapping)
            throws JSONException, ParseException {
            ...
            if (jsonObject.has(spannerColumnName.toUpperCase()) && !jsonObject.isNull(spannerColumnName.toUpperCase())) {
                ...
                switch (dataType) {
                    ...
                    case "timestamp with time zone":
                        Date date = formatter.parse(jsonObject.getString(spannerColumnName.toUpperCase()));
                        mutationBuilder.set(spannerColumnName.toLowerCase()).to(convertTimestamp(date));
                        break;
                    // Please add additional datatype mapping
                    default:
    ...
    public Mutation.WriteBuilder prescriptionMutationBuilder(Mutation.WriteBuilder mutationBuilder, Prescription prescription) {
        return mutationBuilder
            .set("prescription_id")
